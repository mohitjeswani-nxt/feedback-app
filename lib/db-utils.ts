...
export class DatabaseService {
  ...
  static async updateUser(clerkId: string, updates: Partial<User>) {
    ...
    const result = await db
      .collection("users")
      .updateOne({ clerkId }, { $set: { ...updates, updatedAt: new Date() } });
  ...
  static async getTeamById(teamId: string) {
    const db = await this.getDb();
    return await db.collection("teams").findOne({ _id: new ObjectId(teamId) });
  }
  ...
  static async createFeedback(
    feedbackData: Omit<Feedback, "_id" | "ticketId" | "createdAt" | "updatedAt">
  ) {
    const db = await this.getDb();
    const ticketId = `FB-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 4)
      .toUpperCase()}`;
  ...
  static async createFormTemplate(
    templateData: Omit<FormTemplate, "_id" | "createdAt" | "updatedAt">
  ) {
    const db = await this.getDb();
    const template = {
      ...templateData,
