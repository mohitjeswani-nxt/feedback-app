...
export async function GET(request: NextRequest) {
  try {
    ...
    if (format === "csv") {
      ...
      const csvRows = feedbacks.map((feedback) => {
        const resolutionTime =
          feedback.resolvedAt && feedback.assignedAt
            ? Math.ceil((feedback.resolvedAt.getTime() - feedback.assignedAt.getTime()) / (1000 * 60 * 60 * 24))
            : ""
        ...
        return [
          ...
          feedback.slaDeadline ? feedback.slaDeadline.toISOString().split("T")[0] : "",
          feedback.resolvedAt ? feedback.resolvedAt.toISOString().split("T")[0] : "",
          resolutionTime,
      ...
      // Log audit action
      await logAuditAction(currentUser._id, "data_export", `Exported ${feedbacks.length} feedback records as CSV`, {
        format,
        recordCount: feedbacks.length,
        filters: { status, program, startDate, endDate },
      })
