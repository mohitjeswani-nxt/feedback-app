...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (role === "co-admin" && currentUser.role !== "admin") {
      return NextResponse.json({ error: "Only admin can manage co-admin roles" }, { status: 403 })
    }
    ...
    await logAuditAction(
      currentUser._id,
      "user_role_updated",
      `Updated user ${targetUser.name} role from ${oldRole} to ${role}`,
      { targetUserId, oldRole, newRole: role },
    )
