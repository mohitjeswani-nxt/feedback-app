...
export function DynamicFeedbackForm({
  ...
  const submitFeedback = async () => {
    ...
    console.log("Submitting feedback with data:", formData, files);
    setSubmitting(true);
    try {
      // In a real implementation, you'd upload files to a service like UploadThing
      // For now, we'll simulate file URLs using stable identifiers
      const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
      const screenshotUrls = files.screenshots.map(
        (_, index) => `/uploads/screenshots/${timestamp}_${index}.jpg`
      );
      ...
      <Card>
        <CardContent className="text-center py-12">
          <p className="text-muted-foreground">
            Form template not found for {program} program.
          </p>
        </CardContent>
      </Card>
    );
